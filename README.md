...
| Package                                                                                                                                                                                                                                                                                            | What it is                                                                                                                                                                                                                                                                                                   |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`packages/tokens`](packages/tokens)                                                                                                                                                                                                                                                               | Design tokens (color, typography, spacing, elevation, radius, motion) plus the self-hosted Inter/JetBrains Mono typefaces - the single source of truth, built with Style Dictionary. Color is per-business-unit brand (`azure-blue`, `rose-red`, `cyan-orange` - see its README); everything else is shared. |
...
| [`packages/components/core`](packages/components/core), [`button`](packages/components/button), [`form-field`](packages/components/form-field), [`card`](packages/components/card), [`header`](packages/components/header), [`menu`](packages/components/menu), [`icon`](packages/components/icon) | The Angular component library (`brk-` prefixed selectors, e.g. `brk-form-field`, `brkButton`, `brk-menu`, `brk-icon`).                                                                                                                                                                                       |
...
npm install @app-design-system/core @app-design-system/button \
  ...
  @app-design-system/header @app-design-system/menu @app-design-system/icon
...
Two stylesheets are worth calling out because nothing warns you if you skip
them - they fail silently, not loudly:

```css
/* The typefaces the tokens name. Without this, --font-family-sans falls
   through to system-ui and the app quietly isn't in the design system's
   font at all. */
@import '@app-design-system/tokens/css/fonts.css';

/* The icon font and the .brk-icon class. Without this, <brk-icon> renders
   the literal glyph name. */
@import '@app-design-system/icon/css/icons.css';
```
...
Done: tokens (multi-brand), the self-hosted typefaces and the semantic type
scale, theming across Angular Material/Tailwind/Bootstrap, the Angular
component library (button, form-field, card, header, menu, icon), the
Storybook docs site (component stories, brand/theme switchers, and the
Colors / Typography / Spacing & Layout / Elevation & Motion / Iconography
guides), the Tailwind/Bootstrap playgrounds, and GitHub Packages publish
config (Changesets) for every package.

Next: form controls (checkbox, radio, switch, select), then structural
components (tabs, table, dialog, tooltip, accordion) and a footer, all
built on the foundations above.
