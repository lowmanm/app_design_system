...
function generateBrandPalettes() {
  ...
  generateBrandNamesModule();
}

// The brand list has to be readable from TypeScript too (consumers import
// BRAND_NAMES/BrandName from this package). Emitting it here rather than
// hand-maintaining a parallel literal in src/index.ts means BRANDS below is
// the only place a brand is ever declared.
function generateBrandNamesModule() {
  const names = Object.keys(BRANDS);
  const contents = `// AUTO-GENERATED by scripts/generate-palettes.mjs - do not edit directly.
...
`;
  const outFile = resolve(__dirname, '../src/generated-brands.ts');
  writeFileSync(outFile, contents);
  console.log(`Wrote ${outFile} (${names.length} brands)`);
}
