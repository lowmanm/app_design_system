...
import { AdsButtonHarness } from './button.harness';

@Component({
  standalone: true,
  imports: [AdsButtonComponent],
  template: `
    <button adsButton variant="outlined" size="lg" [disabled]="disabled" (click)="onClick()">
      Save
    </button>
  `,
})
...
// AdsButtonHarness's click()/getAttribute(): CDK's ComponentHarness relies
...
describe('AdsButtonComponent', () => {
  ...
  it('applies variant and size classes', () => {
    ...
    fixture.detectChanges();
    const button: HTMLButtonElement = fixture.nativeElement.querySelector('button');
    expect(button.classList.contains('ads-button--outlined')).toBe(true);
    expect(button.classList.contains('ads-button--lg')).toBe(true);
  });
  ...
  it('exposes its text through AdsButtonHarness', async () => {
    const fixture = TestBed.createComponent(HostComponent);
    fixture.detectChanges();
    const harness = await TestbedHarnessEnvironment.harnessForFixture(fixture, AdsButtonHarness);
