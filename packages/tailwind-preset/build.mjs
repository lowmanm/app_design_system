...
const colors = Object.fromEntries(colorKeys.map((key) => [key, cssVar('color', key)]));
const spacing = Object.fromEntries(Object.keys(core.space).map((key) => [key, cssVar('space', key)]));
const borderRadius = Object.fromEntries(Object.keys(core.radius).map((key) => [key, cssVar('radius', key)]));
const boxShadow = Object.fromEntries(
  Object.keys(core.elevation).map((key) => [key, cssVar('elevation', key)]),
);
const fontSize = Object.fromEntries(Object.keys(core.font.size).map((key) => [key, cssVar('font-size', key)]));
const fontFamily = Object.fromEntries(
  Object.keys(core.font.family).map((key) => [key, [cssVar('font-family', key)]]),
);
...
const transitionDuration = Object.fromEntries(
  Object.keys(core.motion.duration).map((key) => [key, cssVar('motion-duration', key)]),
);
const transitionTimingFunction = Object.fromEntries(
  Object.keys(core.motion.easing).map((key) => [key, cssVar('motion-easing', key)]),
);
...
writeFileSync(resolve(__dirname, 'dist/preset.cjs'), `module.exports = ${JSON.stringify(preset, null, 2)};\n`);
writeFileSync(resolve(__dirname, 'dist/preset.mjs'), `export default ${JSON.stringify(preset, null, 2)};\n`);
